...
describe("GameBridge cruiserHostableCosts (issue #7)", () => {
  ...
  test("cost reflects already-owned units (cost ladder advances)", () => {
    const playerLike = makePlayerLike({
      credits: 1_000_000n,
      ownedColonies: 2,
    });
  ...
  test("cost-function throws are NOT swallowed (no silent try/catch)", () => {
    const playerLike = makePlayerLike({
      credits: 50_000n,
      ownedColonies: 0,
    });
    ...
    expect(() => bridge.tick()).toThrow(/boom/);
    bridge.destroy();
  });
});
