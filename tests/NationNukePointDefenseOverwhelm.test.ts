  ...
  test("nation overwhelms enemy SAM with atom bomb salvo on Impossible difficulty", async () => {
    ...
    for (let x = 10; x < 40; x++) {
      for (let y = 10; y < 40; y++) {
        ...
        if (game.map().isSector(tile)) nation.conquer(tile);
      }
    }
    for (let x = 60; x < 90; x++) {
      for (let y = 60; y < 90; y++) {
        const tile = game.ref(x, y);
        if (game.map().isSector(tile)) human.conquer(tile);
      }
    }
    ...
    const sam = human.buildUnit(UnitType.PointDefenseArray, samTile, {});
    game.addExecution(new PointDefenseArrayExecution(human, null, sam));
    ...
    expect(nation.units(UnitType.AntimatterTorpedo)).toHaveLength(0);
    ...
    for (let i = 0; i < 10 && !salvoLaunched; i++) {
      ...
      for (let tick = 0; tick < 150; tick++) {
        ...
        if (nation.units(UnitType.AntimatterTorpedo).length > 0) {
          salvoLaunched = true;
          break;
        }
      }
    ...
    const atomBombs = nation.units(UnitType.AntimatterTorpedo);
