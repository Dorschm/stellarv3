...
describe("Shell Random Damage", () => {
  ...
  test("Shell damage varies randomly between 200-300 base damage", () => {
    ...
    for (let i = 0; i < numShells; i++) {
      const shell = new PlasmaBoltExecution(
        game.ref(coastX, 10),
        player1,
        player1.buildUnit(UnitType.Battlecruiser, game.ref(coastX, 10), {
    ...
    const baseDamage =
      game.config().unitInfo(UnitType.PlasmaBolt).damage ?? 250;
  ...
  test("Shell damage is consistent with same random seed", () => {
    ...
    const shell1 = new PlasmaBoltExecution(
      game.ref(coastX, 10),
      player1,
      player1.buildUnit(UnitType.Battlecruiser, game.ref(coastX, 10), {
        patrolTile: game.ref(coastX, 10),
    ...
    const shell2 = new PlasmaBoltExecution(
      game.ref(coastX, 10),
      player1,
      player1.buildUnit(UnitType.Battlecruiser, game.ref(coastX, 10), {
        patrolTile: game.ref(coastX, 10),
