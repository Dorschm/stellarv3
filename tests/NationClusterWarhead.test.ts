...
describe("Nation MIRV Retaliation", () => {
  test("nation retaliates with MIRV when attacked by MIRV", async () => {
    ...
    attacker.buildUnit(UnitType.OrbitalStrikePlatform, game.ref(10, 10), {});
    ...
    nation.buildUnit(UnitType.OrbitalStrikePlatform, game.ref(50, 50), {});
    // Register OrbitalStrikePlatformExecution so the silo can reload after firing
    const nationSilo = nation.units(UnitType.OrbitalStrikePlatform)[0];
    if (nationSilo) {
    ...
    expect(attacker.units(UnitType.OrbitalStrikePlatform)).toHaveLength(1);
    expect(nation.units(UnitType.OrbitalStrikePlatform)).toHaveLength(1);
    expect(attacker.credits()).toBeGreaterThan(35_000_000n);
    expect(nation.credits()).toBeGreaterThan(35_000_000n);

    // Track MIRVs before nation retaliates
    const mirvCountBefore = nation.units(UnitType.ClusterWarhead).length;
    ...
    for (const gameId of gameIds) {
      ...
      for (let tick = 0; tick < 200; tick++) {
        ...
        if (nation.units(UnitType.ClusterWarhead).length > mirvCountBefore) {
    ...
    const mirvCountAfter = nation.units(UnitType.ClusterWarhead).length;
    expect(mirvCountAfter).toBeGreaterThan(mirvCountBefore);

    // Verify the retaliatory MIRV targets the attacker's territory
    const nationMirvs = nation.units(UnitType.ClusterWarhead);
  ...
  test("nation launches MIRV to prevent victory when player approaches win condition", async () => {
    ...
    if (nationTile) {
      nation.buildUnit(UnitType.OrbitalStrikePlatform, nationTile, {});
      // Register OrbitalStrikePlatformExecution so the silo can reload after firing
      const silo = nation.units(UnitType.OrbitalStrikePlatform)[0];
      if (silo) {
    ...
    // Verify preconditions
    expect(dominantPlayer.units(UnitType.OrbitalStrikePlatform)).toHaveLength(
      0,
    );
    expect(nation.units(UnitType.OrbitalStrikePlatform)).toHaveLength(1);
    expect(nation.units(UnitType.ClusterWarhead)).toHaveLength(0);
    expect(dominantPlayer.units(UnitType.ClusterWarhead)).toHaveLength(0);
    expect(dominantPlayer.credits()).toBeGreaterThan(35_000_000n);
    expect(nation.credits()).toBeGreaterThan(35_000_000n);
  ...
  test("nation launches MIRV to stop steamrolling player with excessive cities", async () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (secondPlayerTile) {
        secondPlayer.buildUnit(UnitType.Colony, secondPlayerTile, {});
      }
    }
    ...
    for (let i = 0; i < minLeaderCities + 2; i++) {
      ...
      if (steamrollerTile) {
        steamroller.buildUnit(UnitType.Colony, steamrollerTile, {});
      }
    }
    ...
    // Verify preconditions
    expect(nation.units(UnitType.OrbitalStrikePlatform)).toHaveLength(1);
    expect(steamroller.unitCount(UnitType.Colony)).toBe(minLeaderCities + 2);
    expect(secondPlayer.unitCount(UnitType.Colony)).toBe(5);
    expect(nation.units(UnitType.ClusterWarhead)).toHaveLength(0);
  ...
  test("nation does not launch MIRV for steamroll when leader has <= 10 cities", async () => {
    ...
    // Verify preconditions
    expect(nation.units(UnitType.OrbitalStrikePlatform)).toHaveLength(1);
    expect(steamroller.unitCount(UnitType.Colony)).toBe(minLeaderCities);
    expect(secondPlayer.unitCount(UnitType.Colony)).toBe(5);
    expect(nation.units(UnitType.ClusterWarhead)).toHaveLength(0);
  ...
  test("nation launches MIRV to prevent team victory when team approaches victory denial threshold (targets biggest team member)", async () => {
    ...
    expect(nation.units(UnitType.OrbitalStrikePlatform)).toHaveLength(1);
    expect(nation.units(UnitType.ClusterWarhead)).toHaveLength(0);
    expect(teamPlayer1.credits()).toBeGreaterThan(35_000_000n);
    expect(teamPlayer2.credits()).toBeGreaterThan(35_000_000n);
    expect(nation.credits()).toBeGreaterThan(35_000_000n);
    expect(nation.isAlive()).toBe(true);
    expect(nation.numTilesOwned()).toBeGreaterThan(0);
