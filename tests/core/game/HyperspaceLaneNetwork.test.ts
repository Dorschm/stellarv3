...
import { HyperspaceLane } from "../../../src/core/game/HyperspaceLane";
import {
  HyperspaceLaneNetworkImpl,
  StationManagerImpl,
} from "../../../src/core/game/HyperspaceLaneNetworkImpl";
import { Cluster } from "../../../src/core/game/TradeHub";

// Mock types
const createMockStation = (unitId: number): any => {
  const cluster = new Cluster();
  const lane = new Set<HyperspaceLane>();
  return {
    unit: {
      id: unitId,
      setTradeHub: vi.fn(),
...
describe("HyperspaceLaneNetworkImpl", () => {
  let network: HyperspaceLaneNetworkImpl;
  ...
  beforeEach(() => {
    ...
    network = new HyperspaceLaneNetworkImpl(game, stationManager, pathService);
  });
  ...
  describe("computeGhostHyperspaceLanePaths", () => {
    test("returns empty when snappable rails exist nearby", () => {
      const tile = 42 as any;
      // Accessing private hyperspaceLaneGrid via any to set up mock
      const hyperspaceLaneGridMock = { query: vi.fn(() => new Set([{}])) };
      (network as any).hyperspaceLaneGrid = hyperspaceLaneGridMock;

      const result = network.computeGhostHyperspaceLanePaths(
        UnitType.Colony,
        tile,
      );
      expect(result).toEqual([]);
      expect(hyperspaceLaneGridMock.query).toHaveBeenCalledWith(tile, 3);
    });
