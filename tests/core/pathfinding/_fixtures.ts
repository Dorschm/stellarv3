...
export const W = "W"; // Deep space (legacy "W" marker kept for fixture readability)
export const L = "L"; // Sector (legacy "L" marker kept for fixture readability)

// Terrain encoding
const DEEP_SPACE_BIT = 0x20;
const SECTOR_BIT = 0x80;
...
function computeShoreline(
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      const idx = y * width + x;
      const isSector = (terrain[idx] & SECTOR_BIT) !== 0;
      const neighbors = [
        [x - 1, y],
        [x + 1, y],
        [x, y - 1],
      ...
      for (const [nx, ny] of neighbors) {
        ...
        const neighborIsSector = (terrain[ny * width + nx] & SECTOR_BIT) !== 0;
        if (isSector !== neighborIsSector) {
...
export function createGame(data: TestMapData): Game {
  ...
  for (let i = 0; i < grid.length; i++) {
    if (grid[i] === L) {
      terrain[i] = SECTOR_BIT;
      numSector++;
    } else {
      terrain[i] = DEEP_SPACE_BIT;
    }
  }

  computeShoreline(terrain, width, height);

  const gameMap = new GameMapImpl(width, height, terrain, numSector);
  ...
  for (let my = 0; my < miniHeight; my++) {
    for (let mx = 0; mx < miniWidth; mx++) {
      ...
      if (hasWater) {
        miniTerrain[mIdx] = DEEP_SPACE_BIT;
      } else {
        miniTerrain[mIdx] = SECTOR_BIT;
        miniNumLand++;
      }
    }
  }
...
export function createGameMap(data: TestMapData): GameMapImpl {
  ...
  return new GameMapImpl(width, height, terrain, numSector);
}
