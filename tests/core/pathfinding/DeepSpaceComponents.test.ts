...
describe("ConnectedComponents", () => {
  describe("getComponentId", () => {
    ...
    it("returns same component ID for all water tiles in single connected area", () => {
      ...
      expect(map.isDeepSpace(water1)).toBe(true);
      expect(map.isDeepSpace(water2)).toBe(true);
      expect(map.isDeepSpace(water3)).toBe(true);
      expect(map.isDeepSpace(water4)).toBe(true);
    ...
    it("returns different component IDs for disconnected water areas", () => {
      ...
      expect(map.isDeepSpace(leftWater1)).toBe(true);
      expect(map.isDeepSpace(leftWater2)).toBe(true);
      expect(map.isDeepSpace(rightWater1)).toBe(true);
      expect(map.isDeepSpace(rightWater2)).toBe(true);
      ...
      expect(leftId1).not.toBe(SECTOR_MARKER);

      expect(rightId1).toBe(rightId2);
      expect(rightId1).toBeGreaterThan(0);
      expect(rightId1).not.toBe(SECTOR_MARKER);
    });

    it("returns SECTOR_MARKER for land tiles", () => {
      const map = createGameMap(twoComponentsMapData);
      const wc = new ConnectedComponents(map);
      wc.initialize();
      ...
      expect(map.isSector(landTile1)).toBe(true);
      expect(map.isSector(landTile2)).toBe(true);
      expect(map.isSector(landTile3)).toBe(true);

      expect(wc.getComponentId(landTile1)).toBe(SECTOR_MARKER);
      expect(wc.getComponentId(landTile2)).toBe(SECTOR_MARKER);
      expect(wc.getComponentId(landTile3)).toBe(SECTOR_MARKER);
    });
  });
