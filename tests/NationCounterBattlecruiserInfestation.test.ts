...
describe("Counter Warship Infestation", () => {
  test("rich nation sends counter-warship in FFA when enemy has too many warships", async () => {
    ...
    expect(nation.units(UnitType.Spaceport)).toHaveLength(1);
    expect(enemy.units(UnitType.Battlecruiser).length).toBeGreaterThan(10);
    expect(game.unitCount(UnitType.Battlecruiser)).toBeGreaterThan(10);
    expect(nation.credits()).toBeGreaterThan(0n);
    ...
    for (const gameId of gameIds) {
      ...
      // (first hit initializes behaviors, second hit runs counterBattlecruiserInfestation)
  ...
  test("rich nation sends counter-warship in Team game when enemy team has too many warships", async () => {
    ...
    // Verify preconditions
    expect(nation.units(UnitType.Spaceport)).toHaveLength(1);
    expect(enemy1.units(UnitType.Battlecruiser).length).toBe(10);
    expect(enemy2.units(UnitType.Battlecruiser).length).toBe(6);
    const totalEnemyTeamWarships =
      enemy1.units(UnitType.Battlecruiser).length +
      enemy2.units(UnitType.Battlecruiser).length;
