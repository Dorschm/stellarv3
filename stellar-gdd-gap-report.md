...
**Last updated:** 2026-05-13 (build cap removal + Colony-gated claim row clarified: deep-space gate + no in-sector claims)
...
| 2     | Limited slots per planet, stackable levels                        | DEVIATION | Upgrade levels MATCH. Per-planet hard build cap intentionally removed — cost-scaling (`2^n × base`) is now the sole economic gate. See `docs/product-decisions.md` §6.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| 4     | Control rules (0/1/2 structures)                                  | DEVIATION | Hard build cap removed in May 2026 balance pass; cost-scaling is the only gate. See `docs/product-decisions.md` §6.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| 14    | Colony-gated territory expansion                                  | DEVIATION | A Battlecruiser claims only when (a) it is carrying a Colony in its one slot AND (b) it is itself operating in deep space — `BattlecruiserExecution.claimTerritoryRadius` checks both gates before doing anything. When both are satisfied it terraforms `DeepSpace` tiles within `battlecruiserTerritoryRadius` into `AsteroidField` sector tiles and conquers any unowned sector tiles in the same radius for the cruiser owner. A cruiser without a Colony slotted does NOT claim or terraform; a cruiser whose current tile is inside an existing sector also does NOT claim, even if it is carrying a Colony — the deep-space gate is what makes Colony-carriers mobile _sector-openers_ rather than in-sector annexers. Documented in `docs/product-decisions.md`. |
...
- ~~**Per-planet slot cap (GDD §4 "0/1/2 structures per planet").**~~
  Resolved — the hard build cap was removed in the May 2026 balance
  pass; cost scaling is now the only economic gate (see
  `docs/product-decisions.md` §6). Listed as `DEVIATION (accepted,
build cap removed)` in §3.
...
| Per-planet slot cap final number (GDD §4 says 0/1/2 — confirm range)    | DECIDED  | Hard build cap removed; see `docs/product-decisions.md` §6 |
