...
function SettingToggle({
  ...
  return (
    <label className="flex items-center justify-between py-3 px-4 rounded-lg bg-white/5 border border-white/10 cursor-pointer hover:bg-white/10 transition-colors">
      ...
      <div
        className={`relative w-10 h-5 rounded-full transition-colors ${checked ? "bg-blue-600" : "bg-white/20"}`}
      ...
      <ModalContainer>
        ...
        <div className="flex border-b border-white/10 px-4">
          <button
            ...
            className={`px-4 py-2 text-sm font-medium uppercase tracking-wider transition-colors ${activeTab === "basic" ? "text-blue-400 border-b-2 border-blue-400" : "text-white/50 hover:text-white/80"}`}
          >
            {translateText("settings.basic")}
          </button>
          <button
            onClick={() => setActiveTab("keybinds")}
            className={`px-4 py-2 text-sm font-medium uppercase tracking-wider transition-colors ${activeTab === "keybinds" ? "text-blue-400 border-b-2 border-blue-400" : "text-white/50 hover:text-white/80"}`}
          >
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-4 flex flex-col gap-2 scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent">
          {activeTab === "basic" ? (
            <>
              <SettingToggle
                label={translateText("settings.dark_mode")}
                checked={userSettings.darkMode()}
                onChange={makeToggle(() => userSettings.toggleDarkMode())}
              />
              <SettingToggle
                label={translateText("settings.emojis")}
                checked={userSettings.emojis()}
                onChange={makeToggle(() => userSettings.toggleEmojis())}
              />
              <SettingToggle
                label={translateText("settings.alert_frame")}
                checked={userSettings.alertFrame()}
                onChange={makeToggle(() => userSettings.toggleAlertFrame())}
              />
              <SettingToggle
                label={translateText("settings.fx_layer")}
                checked={userSettings.fxLayer()}
                onChange={makeToggle(() => userSettings.toggleFxLayer())}
              />
              <SettingToggle
                label={translateText("settings.territory_patterns")}
                checked={userSettings.territoryPatterns()}
                onChange={makeToggle(() =>
                  userSettings.toggleTerritoryPatterns(),
                )}
              />
              <SettingToggle
                label={translateText("settings.cursor_cost_label")}
                checked={userSettings.cursorCostLabel()}
                onChange={makeToggle(() =>
                  userSettings.toggleCursorCostLabel(),
                )}
              />
              <SettingToggle
                label={translateText("settings.anonymous_names")}
                checked={userSettings.anonymousNames()}
                onChange={makeToggle(() => userSettings.toggleRandomName())}
              />
              <SettingToggle
                label={translateText("settings.lobby_id_visibility")}
                checked={userSettings.lobbyIdVisibility()}
                onChange={makeToggle(() =>
                  userSettings.toggleLobbyIdVisibility(),
                )}
              />
              <SettingToggle
                label={translateText("settings.left_click_opens_menu")}
              ...
              />
              <SettingToggle
                label={translateText("settings.performance_overlay")}
                checked={userSettings.performanceOverlay()}
                onChange={makeToggle(() =>
              ...
              <div className="py-3 px-4 rounded-lg bg-white/5 border border-white/10">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-white/80 text-sm">
                    {translateText("settings.attack_ratio")}
                  </span>
                  <span className="text-white/60 text-xs">
                    {userSettings.attackRatioIncrement()}
                  </span>
                </div>
                <input
                  type="range"
            ...
            <div className="text-white/60 text-sm space-y-2">
              <p className="text-white/40 text-xs uppercase tracking-wider mb-3">
                {translateText("settings.keybinds_desc")}
              </p>
              <KeybindSettings />
            </div>
          )}
...
function KeybindSettings() {
  ...
  const keybindEntries: { action: string; label: string }[] = [
    {
      action: "toggleView",
      label: translateText("settings.keybind_toggle_view"),
    },
    {
      action: "coordinateGrid",
      label: translateText("settings.keybind_coordinate_grid"),
    },
    {
      action: "centerCamera",
      label: translateText("settings.keybind_center_camera"),
    },
    ...
    {
      action: "moveRight",
      label: translateText("settings.keybind_move_right"),
    },
    ...
    { action: "zoomIn", label: translateText("settings.keybind_zoom_in") },
    {
      ...
      label: translateText("settings.keybind_attack_ratio_down"),
    },
    {
      action: "attackRatioUp",
      label: translateText("settings.keybind_attack_ratio_up"),
    },
    {
      action: "swapDirection",
      label: translateText("settings.keybind_swap_direction"),
    },
    { action: "resetGfx", label: translateText("settings.keybind_reset_gfx") },
    {
      action: "pauseGame",
      label: translateText("settings.keybind_pause_game"),
    },
    {
      action: "gameSpeedUp",
      label: translateText("settings.keybind_game_speed_up"),
    },
    {
      action: "gameSpeedDown",
      label: translateText("settings.keybind_game_speed_down"),
    },
  ];
