  ...
  const setFlag = useCallback(
    (flag: string) => {
      ...
      document.dispatchEvent(
        new CustomEvent("event:user-settings-changed:flag"),
      );
      showPage("page-play");
    },
    ...
    <ModalPage pageId="flag-input-modal" onOpen={onOpen} onClose={onClose}>
      <ModalContainer>
        <div className="flex items-center gap-3 px-4 py-3 border-b border-white/10 shrink-0">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-lg bg-blue-600/20 text-blue-400 border border-blue-500/30 text-xs font-medium hover:bg-blue-600/30 transition-colors"
            >
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-4 scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent">
          ...
          <button
            onClick={() => setFlag("")}
            className="mb-3 px-4 py-2 rounded-lg bg-white/5 border border-white/10 text-white/60 hover:bg-white/10 transition-colors text-sm w-full text-left"
          >
          ...
          {cosmeticFlags.length > 0 && (
            <div className="mb-4">
              <p className="text-xs text-white/40 uppercase tracking-wider mb-2">
                {translateText("flag_input.cosmetic_flags")}
              </p>
              <div className="grid grid-cols-5 sm:grid-cols-8 gap-2">
                {cosmeticFlags.map((flag) => (
                  <button
                    key={flag.name}
                    onClick={() => setFlag(`flag:${flag.name}`)}
                    className="flex flex-col items-center gap-1 p-1.5 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 transition-colors"
                    title={flag.name}
                  >
                    <img
                      src={flag.url}
          ...
          {/* Country flags */}
          <p className="text-xs text-white/40 uppercase tracking-wider mb-2">
            {translateText("flag_input.country_flags")}
          </p>
          <div className="grid grid-cols-5 sm:grid-cols-8 gap-2">
            {filteredCountries.map((country) => (
              <button
                key={country.code}
                onClick={() => setFlag(`country:${country.code}`)}
                className="flex flex-col items-center gap-1 p-1.5 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 transition-colors"
                title={country.name}
              >
                <img
