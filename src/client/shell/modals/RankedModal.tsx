...
export function RankedModal() {
  ...
  const [userMe, setUserMe] = useState<
    Awaited<ReturnType<typeof getUserMe>> | false
  >(false);
  ...
  const onOpen = useCallback(async () => {
    ...
    try {
      ...
      if (hasLinkedAccount(response) && response !== false) {
        setElo(
          response.player.leaderboard?.oneVone?.elo ??
  ...
  const renderCard = (title: string, subtitle: string, onClick: () => void) => (
    <button
      onClick={onClick}
      className="flex flex-col w-full h-28 sm:h-32 rounded-2xl bg-[color-mix(in_oklab,var(--frenchBlue)_70%,black)] border-0 transition-transform hover:scale-[1.02] active:scale-[0.98] p-6 items-center justify-center gap-3"
    >
      <div className="flex flex-col items-center gap-1 text-center">
        <h3 className="text-lg sm:text-xl font-bold text-white uppercase tracking-widest leading-tight">
          {title}
        </h3>
        <p className="text-xs text-white/60 uppercase tracking-wider whitespace-pre-line leading-tight">
          {subtitle}
        </p>
      </div>
    </button>
  ...
  const renderDisabledCard = (title: string, subtitle: string) => (
    <div className="flex flex-col w-full h-28 sm:h-32 overflow-hidden rounded-2xl bg-slate-900/40 backdrop-blur-md border-0 p-6 items-center justify-center gap-3 opacity-50 cursor-not-allowed">
      <div className="flex flex-col items-center gap-1 text-center">
        <h3 className="text-lg sm:text-xl font-bold text-white/60 uppercase tracking-widest leading-tight">
          {title}
        </h3>
        <p className="text-xs text-white/40 uppercase tracking-wider whitespace-pre-line leading-tight">
          {subtitle}
        </p>
      </div>
    </div>
