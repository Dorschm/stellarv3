      ...
      <ModalContainer>
        <div className="flex items-center gap-3 px-4 py-3 border-b border-white/10 shrink-0">
          ...
          <h2 className="text-lg font-bold text-white uppercase tracking-widest">
            {translateText("main.leaderboard")}
          </h2>
        </div>
        ...
        <div className="flex border-b border-white/10 px-4">
          <button
            ...
            className={`px-4 py-2 text-sm font-medium uppercase tracking-wider transition-colors ${activeTab === "players" ? "text-blue-400 border-b-2 border-blue-400" : "text-white/50 hover:text-white/80"}`}
          >
            {translateText("leaderboard.players")}
          </button>
          <button
            onClick={() => handleTabChange("clans")}
            className={`px-4 py-2 text-sm font-medium uppercase tracking-wider transition-colors ${activeTab === "clans" ? "text-blue-400 border-b-2 border-blue-400" : "text-white/50 hover:text-white/80"}`}
          >
              ...
              <thead>
                <tr className="text-white/40 text-xs uppercase tracking-wider text-left border-b border-white/10">
                  ...
                  <th className="py-2 pr-4 text-right">
                    {translateText("leaderboard.score")}
                  </th>
                </tr>
              </thead>
              <tbody>
                {players.map((p) => (
                  ...
                  >
                    <td className="py-2.5 pl-4 text-white/40 font-mono">
                      {p.rank}
                    </td>
                    <td className="py-2.5 text-white">{p.username}</td>
                    <td className="py-2.5 pr-4 text-right text-white/70 font-mono">
                ...
                {players.length === 0 && (
                  <tr>
                    <td colSpan={3} className="text-center text-white/40 py-8">
                      {translateText("leaderboard.no_data")}
                    </td>
                  </tr>
                )}
              </tbody>
              ...
              <thead>
                <tr className="text-white/40 text-xs uppercase tracking-wider text-left border-b border-white/10">
                  ...
                  <th className="py-2 text-right">
                    {translateText("leaderboard.members")}
                  </th>
                  <th className="py-2 pr-4 text-right">
                    {translateText("leaderboard.score")}
                  </th>
