...
export function TerritoryPatternsModal() {
    ...
    (patternName: string) => {
      ...
      window.dispatchEvent(
        new CustomEvent("show-message", {
          detail: {
            message: `Pattern selected: ${patternName}`,
            color: "green",
            duration: 1500,
          },
        }),
      ...
      document.dispatchEvent(
        new CustomEvent("event:user-settings-changed:pattern"),
      );
      showPage("page-play");
    },
  ...
  const ownedPatterns = cosmetics
    ? Object.values(cosmetics.patterns).filter((p) => {
        if (patternRelationship(p, null, userMe, null) !== "owned")
          return false;
  ...
  return (
    <ModalPage
      pageId="territory-patterns-modal"
      onOpen={onOpen}
      onClose={onClose}
    >
      <ModalContainer>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-4 scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent">
          ...
          {ownedPatterns.length === 0 && (
            <div className="text-center text-white/40 py-8">
              ...
              <button
                onClick={() => showPage("page-item-store")}
                className="mt-3 px-4 py-2 rounded-lg bg-blue-600/20 text-blue-400 border border-blue-500/30 text-sm hover:bg-blue-600/30 transition-colors"
              >
