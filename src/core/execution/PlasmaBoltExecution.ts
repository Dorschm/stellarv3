...
export class PlasmaBoltExecution implements Execution {
  private active = true;
  ...
  tick(ticks: number): void {
    this.shell ??= this._owner.buildUnit(UnitType.PlasmaBolt, this.spawn, {});
    ...
    if (this.destroyAtTick === -1 && !this.ownerUnit.isActive()) {
      this.destroyAtTick =
        this.mg.ticks() + this.mg.config().plasmaBoltLifetime();
    }
  ...
  private effectOnTarget(): number {
    const { damage } = this.mg.config().unitInfo(UnitType.PlasmaBolt);
