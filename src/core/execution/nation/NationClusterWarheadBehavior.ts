...
export class NationClusterWarheadBehavior {
  ...
  considerMIRV(): boolean {
    ...
    if (this.player.units(UnitType.OrbitalStrikePlatform).length === 0) {
      return false;
    }
    if (this.player.credits() < this.cost(UnitType.ClusterWarhead)) {
      return false;
    }
  ...
  private maybeSendMIRV(enemy: Player): void {
    ...
    if (
      centerTile &&
      this.player.canBuild(UnitType.ClusterWarhead, centerTile)
    ) {
  ...
  private countCities(p: Player): number {
    return p.unitCount(UnitType.Colony);
  }
