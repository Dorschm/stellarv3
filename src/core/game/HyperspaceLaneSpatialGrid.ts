...
export class HyperspaceLaneSpatialGrid {
  private cells = new Map<string, Set<HyperspaceLane>>();
  // Quick access to avoid iterating over the cells
  private railToCells = new Map<HyperspaceLane, Set<string>>();
  ...
  unregister(rail: HyperspaceLane) {
  ...
  query(tile: TileRef, radius: number): Set<HyperspaceLane> {
    ...
    const result = new Set<HyperspaceLane>();
