...
export class AStarHyperspaceLane implements PathFinder<number> {
...
class RailAdapter implements AStarAdapter {
  ...
  neighbors(node: number, buffer: Int32Array): number {
    ...
    const fromShoreline = this.gameMap.isSectorBoundary(node);
  ...
  private isTraversable(to: number, fromShoreline: boolean): boolean {
    const toWater = this.gameMap.isDeepSpace(to);
    if (!toWater) return true;
    return fromShoreline || this.gameMap.isSectorBoundary(to);
  }

  cost(from: number, to: number, prev?: number): number {
    const penalized =
      this.gameMap.isDeepSpace(to) || this.gameMap.isSectorBoundary(to);
