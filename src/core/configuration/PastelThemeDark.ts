...
export class PastelThemeDark extends PastelTheme {
  private darkSectorEdge = colord("rgb(134,133,88)");

  private darkDeepSpace = colord("rgb(14,11,30)");
  private darkSectorBoundaryDeepSpace = colord("rgb(50,50,50)");
  ...
  terrainColor(gm: GameMap, tile: TileRef): Colord {
    ...
    if (gm.isSectorEdge(tile)) {
      return this.darkSectorEdge;
    }
    switch (gm.terrainType(tile)) {
      case TerrainType.DeepSpace:
      case TerrainType.DebrisField: {
        const w = this.darkDeepSpace.rgba;
        if (gm.isSectorBoundary(tile) && gm.isDeepSpace(tile)) {
          return this.darkSectorBoundaryDeepSpace;
        }
